  ...
  getPerformances(): void {
    this.authService.getPerformances().subscribe({
      next: (data: Performance[]) => {
        console.log('Performances data:', data);
        this.performances = data;
        this.filteredPerformances = data; // Başlangıçta tüm performans verilerini göster
        this.firstNames = Array.from(
          new Set(data.map((item: Performance) => item.firstname))
        ).map((firstname) => ({ firstname })); // Unique First Names
        this.loading = false;
      },
    ...
    if (this.performanceForm.valid) {
      ...
      this.authService.addPerformance(newPerformance).subscribe({
        next: (data) => {
          ...
          this.filterByDate(); // Performans eklendiğinde filtreyi yeniden uygula
      ...
      this.authService
        ...
        .subscribe({
          next: (data) => {
            ...
            this.filterByDate(); // Performans güncellendiğinde filtreyi yeniden uygula
            this.performanceForm.reset();
